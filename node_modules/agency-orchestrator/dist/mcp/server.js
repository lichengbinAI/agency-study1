...
/** 递归查找 YAML 文件 */
function findYamlFiles(dir, result) {
...
export async function startServer(verbose = false) {
    ...
    // ─── Tool 1: run_workflow ───
    server.tool('run_workflow', 'Execute a YAML workflow with the DAG engine', {
        ...
        try {
            ...
            const result = await silentCall(() => run(absPath, (inputs || {}), {
                quiet: true,
    ...
    server.tool('validate_workflow', 'Validate a workflow YAML without executing', {
        path: z.string().describe('Path to workflow YAML file'),
    ...
    server.tool('list_workflows', 'List available workflow templates from the workflows/ directory', {}, async () => {
        try {
    ...
    server.tool('plan_workflow', 'Show the DAG execution plan for a workflow', {
        path: z.string().describe('Path to workflow YAML file'),
    }, async ({ path: workflowPath }) => {
        try {
            const absPath = resolve(workflowPath);
    ...
    server.tool('compose_workflow', 'Generate a workflow YAML from a natural language description using AI', {
    ...
    // ─── Tool 6: list_roles ───
    server.tool('list_roles', 'List available AI roles from the agents directory', {
